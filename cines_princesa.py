...
def scrape_cines_princesa():
    # URL de la página de cine que vas a scrapear
    URL = 'https://www.cinesrenoir.com/cine/cines-princesa/cartelera/'
    ...
    if response.status_code == 200:
        ...
        if peliculas:
            ...
            # Retornar el DataFrame si se encontraron películas
            return df
        else:
            print("No se encontraron películas en la página.")
            return pd.DataFrame()  # Retorna un DataFrame vacío si no se encuentran películas
    else:
        print(f"Error al conectarse a la página web: {response.status_code}")
        return pd.DataFrame()  # Retorna un DataFrame vacío si hay error de conexión

# Si quieres probar el script directamente
if __name__ == "__main__":
    scrape_cines_princesa()
